...
def load_dataset_master(format, name, dataset_dir):
    ...
    for key, pecfg in cfg.items():
        if key.startswith('posenc_') and pecfg is not None and pecfg.enable and (not key.startswith('posenc_ER')):
    ...
    if pe_enabled_list:
        ...
        if not loaded_all_pe_stats:
            ...
            # Save positional encodings so that they are not computed on every run
            include_structural = False
            try:
                include_structural = cfg.posenc_LapPE.eigen.include_high_freqs
            except:
